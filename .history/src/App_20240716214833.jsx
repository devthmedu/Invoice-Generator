...
function App() {
  ...
  useEffect(() => {
    ...
    const dataFormatada = `${String(atual.getDate()).padStart(2, '0')}/${String(atual.getMonth() + 1).padStart(2, '0')}/${atual.getFullYear()}`;
  ...
  useEffect(() => {
    ...
    localStorage.setItem('invoiceData', JSON.stringify({ numeroDaFatura, nomeCliente, produtos }));
  ...
  const adicionarProduto = () => {
    if (produto && quantidade > 0 && preco > 0) {
      const newProduto = { produto, quantidade: Number(quantidade), preco: Number(preco) };
      if (editIndex !== null) {
        const updatedProdutos = produtos.map((item, index) => (index === editIndex ? newProduto : item));
      ...
      toast.error('Por favor, insira um produto válido com quantidade e preço.');
    }
  };
  ...
  const totalFatura = produtos.reduce((acc, curr) => acc + curr.quantidade * curr.preco, 0);
          ...
          <div className="form">
            ...
            <div className="buttons">
              <button
                ...
                title={loading ? 'Aguarde enquanto a fatura é criada' : 'Criar fatura'}
              >
            ...
            <ul>
              {produtos.map((item, index) => (
                <li key={index}>
                  ...
                  <button onClick={() => editarProduto(index)}><FaEdit /></button>
                  <button onClick={() => removerProduto(index)}><FaTrash /></button>
                </li>
              ))}
            </ul>
